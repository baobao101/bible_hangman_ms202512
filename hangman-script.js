...
async function loadWordList() {
    try {
        ...
        console.error("Error loading git json. Using local json.", error);
        ...
        try {
            ...
            console.error("Error loading local json. Using fallback words.", error);
...
async function handleHintRequest() {
    ...
    if (currentScore < HINT_COST) {
        hintDisplay.textContent = `Not enough points (${HINT_COST} required) for a hint!`;
        return;
    }
...
function createLetterButtons() {
    ...
    const alphabet = 'AEIOUY-BCDFGHJKLMNPQRSTVWXZ';
...
function startGame() {
    ...
    if (hintButton) {
        ...
        hintButton.textContent = `Buy Hint (${HINT_COST} points)`;
    }
